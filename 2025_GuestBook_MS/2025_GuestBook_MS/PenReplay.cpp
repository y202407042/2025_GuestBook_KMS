...
void PenReplay::AddPoint(POINT pt)
{
	...
	{
		/// original 벡터가 비어있거나,
		/// 마지막 좌표값과 현재 좌표값이 다른 경우에
		/// 새로운 좌표를 original 벡터에 저장
...
{
	if (!original.empty())										/// 리플레이 원본 좌표가 남아있을 시 실행
	{
		replayBuffer.push_back(original.front());				/// 원본 좌표값을 replayBuffer에 추가
		original.erase(original.begin());						/// 원본 맨 앞 좌표값 제거
		InvalidateRect(hWnd, NULL, TRUE);						/// 영역 갱신
	}
	else
	{
		Stop(hWnd);
